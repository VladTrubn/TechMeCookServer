...
namespace TechMeCookServer.Controllers
{
    ...
    public class RecipesController : Controller
    {
        ...
        private String filteredCollectionUriTemplate = "recipes/complexSearch?query={0}&includeIngredients={1}&equipment={2}&diet={3}&intolerances={4}";
        ...
        [HttpGet("complex")]
        public async Task<IActionResult> GetFilteredCollection(String? query, String? includeIngredients, String? equipment, String? diet, String? intolerances, String apiKey)
        {
            String actualUri = string.Format(randomCollectionUriTemplate, query, includeIngredients, equipment,  diet, intolerances, httpClientService.GetApiKey());
            var response = await httpClient.GetAsync(actualUri);
            response.Content.Headers.ContentType = new MediaTypeHeaderValue("application/json");
            response.EnsureSuccessStatusCode();
            return Ok(await response.Content.ReadAsStringAsync());
